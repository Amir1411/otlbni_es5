    ...
    if (checkblank == 1) {
        ...
        connection.query(pending_order_url, [place_id,status], function(err, result) {
            if (err) {
                ...
                if ( access_token != "" ) {
                    var user_sql = "SELECT `account_balance` FROM `user` WHERE `access_token`=?";
                    connection.query(user_sql, [access_token], function(err, userPlaceResult){
                        if (err) {
                            responses.sendError(res);
                            return;
                        } else {
                            if ( userPlaceResult[0].account_balance == '' ) {
                                var acc = 0;
                            } else {
                                var acc = parseInt(userPlaceResult[0].account_balance);
                            }
                            ...
                            if ( acc > 600 ) {
                                ...
                                getPendingOrderList(userByArray, result, req, res);
                            }
                        }
                    });
                } else {
...
function getPendingOrderList(userByArray, result, req, res) {
...
function get_myorderinactive_details(user_id, req, res, callback) {
    ...
    var my_order_url = "SELECT * FROM `order_details` WHERE `created_by_id`=? AND `status` NOT IN ("+pending_status+","+accept_status+","+ontheway_status+") ORDER BY `row_id` DESC";
...
function get_myorderactive_details(user_id, req, res, callback) {
    ...
    var my_order_url = "SELECT * FROM `order_details` WHERE `created_by_id`=? AND `status` IN("+pending_status+","+accept_status+","+ontheway_status+") ORDER BY `row_id` DESC";
    connection.query(my_order_url, [user_id], function(err, result) {
        if (err) {
            console.log(err);
            responses.sendError(res);
...
function get_delivered_offer_details(user_id, req, res, callback) {
    ...
    var my_offer_url = "SELECT * FROM `offer` WHERE `offer_created_by_id`=? ORDER BY `row_id` DESC";
    ...
    if (checkblank == 1) {
        ...
        utils.authenticateUser(access_token, function(result) {
            if (result == 0) {
                ...
                // var user_id = result[0].user_id;
                if ( notification_id != '' ) {
                    var sql = "UPDATE `notification` SET `notification_status`='1' WHERE `notification_id`=?";
                    connection.query(sql, [notification_id], function(err, notificationStatusResult) {
                        if (err) {
                            responses.sendError(res);
                            return;
                        } else {
                            getNotification(result, notification_type, notification_type_id, sender_id, req, res);
                        }
                    });
                } else {
                    getNotification(result, notification_type, notification_type_id, sender_id, req, res);
...
function getNotification (result, notification_type, notification_type_id, sender_id, req, res) {
    if ( notification_type == "1" ) {
        ...
        connection.query(check_offer_sql, [1, notification_type_id, offer_created_to_id], function(err, check_offer_result){
            ...
            if (err) {
                ...
                var check_offer_reject_sql = "SELECT * FROM `offer` WHERE `status`=? AND `order_id`=? AND `offer_created_to_id`=?";
                connection.query(check_offer_reject_sql, [3, notification_type_id, offer_created_to_id], function(err, check_offer_reject_result){
                    console.log(err)
                    if (err) {
                        responses.sendError(res);
                        return;
                    } else if ( check_offer_reject_result.length > 0 ){
                        var response = {
                            flag: 5,
                            response: {},
                            message: "The order already has been rejected."
                        };
                        res.status(constants.responseFlags.ACTION_COMPLETE).json(response);
                    } else {
                        var sql = "SELECT * FROM `order_details` WHERE `order_id`=?";
        ...
        utils.authenticateUser(access_token, function(result) {
            if (result == 0) {
                ...
                if (accept_reject_offer_status == "1") {
                    var accept_reject_offer_status1 = "1";
                } else if ( accept_reject_offer_status == "2" ) {
                    var accept_reject_offer_status1 = "3";
                }

                var sql = "UPDATE `offer` SET `status`='"+accept_reject_offer_status1+"' WHERE `offer_id`=?";
