app.controller("order", ['$scope','httpService','API_URL','$window','$rootScope', function ($scope, httpService, API_URL, $window, $rootScope) {
	...
	// For list of user
	$scope.init = function() {
		params = {access_token: access_token, status: ''};
		console.log(params);
		httpService.post( API_URL + '/getAllAdminOrder', params).then(function(response){
			...
			if(response.status == 200){
				...
				$scope.allOrderlist = responseData;
