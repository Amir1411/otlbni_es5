app.controller("on_the_way_order", ['$scope','httpService','API_URL','$window','$rootScope', function ($scope, httpService, API_URL, $window, $rootScope) {
	...
	// For list of user
	$scope.init = function() {
		params = {access_token: access_token, status: 1};
		console.log(params);
		httpService.post( API_URL + '/getAdminOrder', params).then(function(response){
			...
			if(response.status == 200){
				...
				$scope.onTheWayOrderlist = responseData;
				angular.element(document).ready( function () {
					$('#onTheWayOrder').dataTable({
						'paging': true,
						'ordering': true,
						'info': true,
						"bDestroy": true,
